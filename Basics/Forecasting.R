Amtrak<-read.csv('C:/Users/Mayuri/Desktop/R/Amtrak.csv') # read the Amtrack data
...
res<-predict(arima(resid,order=c(1,0,0)),n.ahead = 120)
str(pred_res)
pred_res$pred
acf(k$residuals)
####################### Predicting new data #############################
...
pred_test_data<-read_excel(file.choose(),1) #Load Predict_new.xlsx
View(pred_test_data)